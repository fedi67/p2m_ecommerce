...
from models import Product, Customer, ProductVariant
...
async def get_admin_stats(user: Customer = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    ).join(ProductVariant).join(OrderItem).group_by(Product.category).all()

    # Recent Global Feedback
    recent_feedbacks = db.query(GlobalFeedback).order_by(GlobalFeedback.created_at.desc()).limit(5).all()
...
async def create_product(
    ...
    if not user.is_admin:
        raise HTTPException(status_code=403, detail="Accès réservé.")
    from models import Product
    new_p = Product(name=data.name, description=data.description)
    db.add(new_p)
    db.commit()
    db.refresh(new_p)
    return {"id": new_p.id, "message": "Produit créé. Ajoutez maintenant des variantes."}
...
async def delete_product(
    ...
    from models import Product, ProductVariant, OrderItem
    ...
    try:
        ...
        try:
            ...
            if "JSON:" in ai_reply_full:
                ...
                # Si pas de JSON du tout, on affiche les 4 premiers candidats par défaut 
                # pour ne pas laisser Nour avec juste du texte
